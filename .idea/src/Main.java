...
public class Main {
    static Boolean abortOperations = false;//почему задаем по дефолту false ?
    static Scanner input = new Scanner(System.in); //почему у этих полей static ?
    static HashMap<Integer, User> ourUsersDB = DataBase.DBUsers();
    static HashMap<Integer, Book> ourBooksDB = DataBase.DBBooks();


    //почему у метода ниже private ?
    //TODO: переменная так может называться isUserInDb, не метод, метод должен называться типа checkUserInDbById
    ...
    //TODO: как можно было сдедать searchUserById метод поиска проще:
//    public void findByIdUser(Map<Integer, String> people, int id) {
//        System.out.println("FOUND" + people.get(id));
//    }
    ...
    //TODO: deleteUserFromDb название и прокидывание эксепшна -- НА БУДУЩЕЕ лучше использовать try catch with Resources
    // чтобы не пробрасывать ошибку через все классы
    public static void deleteUser(Scanner scanner) throws IncorrectIDException {
        System.out.println("Введите id пользователя для удаления: "); //TODO: все константы можем вынести в начало и задать как константы
    ...
    public static void changeUser(Scanner scanner, HashMap<Integer, User> ourUsersDB) throws
        ...
        if (ourUsersDB.containsKey(id)) {
            ...
            } else if (command == 1) { //TODO: используем swith case там где много условий или enum
    ...
    //TODO: пример обработки в методе выше без if/else
//    public Item getFirstItems() {
//        try {
//            return allNames.add(ourUsersDB.get(i).getName());
//        } catch (NullPointerException e) {
//            System.out.println(("Список из класса " + this + " пустой"));
//        }
//        return null;
//    }

    //TODO: проброс ошибки через НА БУДУЩЕЕ try catch with Resources
    //TODO: как можно сделать createNewUser метод проще:
//    public void saveUser(Map<Integer, String> people){
//        int key = people.size() + 1;
//        people.put(key, "NEW USER, man, 34");
...
//    }
    ...
    public static void main(String[] args) throws IncorrectIDException {
        System.out.println(DataBase.DBBooks());
